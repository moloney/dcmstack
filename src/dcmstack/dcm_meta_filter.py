...
class dcm_meta_filter():
    ...
    def _setDictionaries(self, profile):
        ...
        for node in tree.getiterator():
            if not (node.attrib.get('t') is None):
                if node.tag == 'p':
                    ...
                    self._keepGroups[long(
                        node.attrib.get('t'), 16)] = node.text
                elif node.tag == 'r':
    ...
    def contents(self, toReturnVal, *args):
        ...
        if len_args == 0:
            ...
            if ElementName in self._ds:
                ...
                if len_args == 2:
                    if toReturnVal == 0:
                        self._ds[self._currentTag].value = re.sub(
                            r'%s' % args[1], '', de.value)
                    else:
    ...
    def _doAnonymization(self):
        for key in self._tagFunctions:
            ...
            if not val is None and key in self._ds.keys():
                if '@' in val and (not '-@' in val):
                    self._setCurrentTag(key)
                    func, args = self._getFunctionAndArgs(val)
                    if not func is None and not func == 'param':
                    ...
                    self._ds[self._currentTag].value = str(funcHanldle1(
                        1, args1)) + '-' + str(funcHanldle2(1, args2))
